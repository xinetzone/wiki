...
(wiki:myst/syntax/optional)=
# 可选的 MyST 语法

参考：[Optional MyST Syntaxes (myst-parser.readthedocs.io)](https://myst-parser.readthedocs.io/en/latest/syntax/optional.html#)

MyST-Parser 是高度可配置的，利用 [markdown-it-py](https://markdown-it-py.readthedocs.io/en/latest/index) 解析器固有的 "可插拔性"。以下语法是可选的（默认禁用），可以通过 Sphinx `conf.py` 启用（见 [](sphinx/config-options)）。他们的目标一般是增加更多对 Markdown 友好的语法；经常启用和渲染扩展 [CommonMark 规范](https://commonmark.org/) 的 [markdown-it-py 插件](https://markdown-it-py.readthedocs.io/en/latest/md-plugins)。
...
:::{important}
`myst_enable_extensions` 取代了之前的配置选项：
`admonition_enable`, `figure_enable`, `dmath_enable`, `amsmath_enable`, `deflist_enable`, `html_img_enable`
:::

(wiki:myst/syntax/typography)=

## 排版

在 `myst_enable_extensions` 中添加 `"smartquotes"`（在 Sphinx `conf.py` 配置文件中）会自动将标准引号转换为其开放/封闭的变体：

- `'single quotes'`: 'single quotes'
- `"double quotes"`: "double quotes"

在 `myst_enable_extensions` 中添加 `"replacements"`（在 Sphinx `conf.py` 配置文件中）会自动转换一些常见的排版文字
...
数学的解析是通过在 `myst_enable_extensions` 列表选项中，在 Sphinx `conf.py` 配置文件中加入以下一项或两项：

- `"dollarmath"`（默认被添加）for parsing of dollar `$` and `$$` encapsulated math.
- `"amsmath"` (off by default) for direct parsing of [amsmath LaTeX environments](https://ctan.org/pkg/amsmath).

These options enable their respective Markdown parser plugins, as detailed in the [markdown-it plugin guide](https://markdown-it-py.readthedocs.io/en/latest/md-plugins).
...
Definition lists utilise the [markdown-it-py deflist plugin](https://markdown-it-py.readthedocs.io/en/latest/md-plugins), which itself is based on the [Pandoc definition list specification](http://johnmacfarlane.net/pandoc/README.html#definition-lists).
...
Task lists utilise the [markdown-it-py tasklists plugin](https://markdown-it-py.readthedocs.io/en/latest/md-plugins),
...
As discussed [above](syntax/directives), MyST allow for directives to be used such as `image` and `figure` (see [](https://www.sphinx-doc.org/en/master/usage/restructuredtext/basics.html#rst-primer)):
